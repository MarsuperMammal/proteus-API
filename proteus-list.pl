...
$service->getAliasesByHint(
  ...
  SOAP::Data->type( 'int' )->name( 'count' )->value( '10')->attr({xmlns => ''}),
  SOAP::Data->type( 'string' )->name( 'options' )->value( "aw01emcpappq012.ohs.tycoelectronics.com" )->attr({xmlns => ''})
  );

$service->getHostRecordsByHint(
  SOAP::Data->type( 'int' )->name( 'start' )->value( '0' )->attr({xmlns => ''}),
  SOAP::Data->type( 'int' )->name( 'count' )->value( '10')->attr({xmlns => ''}),
  SOAP::Data->type( 'string' )->name( 'options' )->value( "10.125.130.67" )->attr({xmlns => ''})
  )
->result;
